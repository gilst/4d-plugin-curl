...
static CURLcode wait_or_timeout_with_yield(struct Curl_multi *multi, struct events *ev, void (*_PA_YieldAbsolute)(void))
{
    bool done = FALSE;
    CURLMcode mcode;
    CURLcode result = CURLE_OK;
...
static CURLcode easy_events_with_yield(CURLM *multi, void (*_PA_YieldAbsolute)(void))
{
    ...
    return wait_or_timeout_with_yield(multi, &evs, _PA_YieldAbsolute);
}
...
static CURLcode easy_perform_with_yield(struct SessionHandle *data, bool events, void (*_PA_YieldAbsolute)(void))
{
    CURLM *multi;
    CURLMcode mcode;
    CURLcode result = CURLE_OK;
    ...
    /* run the transfer */
    result = events ? easy_events_with_yield(multi, _PA_YieldAbsolute) : easy_transfer_with_yield(multi, _PA_YieldAbsolute);
...
{
    return easy_perform_with_yield(easy, FALSE, _PA_YieldAbsolute);
}
